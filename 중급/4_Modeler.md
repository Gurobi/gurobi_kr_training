...
막히면 펼쳐보되, 먼저 위 "비즈니스 문제 설명"을 스스로 요약해서 Modeler에게 전달해보길 권합니다.

<details>
<summary>펼쳐서 프롬프트 보기</summary>

> 우리 유가공 공장의 하루 생산 계획을 최적화하고 싶어.
>
> 원유를 사서 프리미엄 유제품 음료 하나를 만드는데, 원유를 분리 설비에 넣으면 크림이랑
> 탈지유가 나와. 원유 1배럴당 크림 0.3배럴, 탈지유 0.5배럴이 나오고 나머지는 공정 손실이야.
>
> 크림은 그대로 음료에 섞어도 되는데 품질지수가 90이야. 크림을 단백질 농축 설비에 넣으면
> 품질지수 115짜리 단백질 농축물로 바뀌는데, 크림 1배럴을 넣으면 농축물 0.6배럴이 나와.
>
> 탈지유는 그 자체로는 음료에 못 쓰고, 2차 가공 설비를 거쳐야 품질지수 105짜리 발효
> 농축물이 되고, 이게 음료 재료로 쓰여. 탈지유 1배럴을 넣으면 발효 농축물 0.3배럴이 나와.
>
> 음료는 크림·단백질 농축물·발효 농축물을 부피 기준으로 섞어서 만드는데, 평균 품질지수가
> 94 이상이어야 팔 수 있어. 품질지수는 부피 기준으로 섞으면 그대로 평균이 나는 점수야.
>
> 원유는 하루 최대 30,000배럴까지 살 수 있고, 분리 설비는 하루 40,000배럴까지 처리할 수
> 있어. 단백질 농축 설비는 하루 6,000배럴, 2차 가공 설비는 하루 5,000배럴까지만 처리할
> 수 있어. 음료는 배럴당 7.0(백 달러)의 이익을 내.
>
> 하루 이익을 최대화하는 생산 계획을 세워 줘.

</details>

### 진행 방법

1. 위 프롬프트를 Intelligence Hub의 **Modeler**에 전달합니다.
2. Modeler가 명세를 구체화하기 위해 던지는 질문에 하나씩 답합니다.
3. 명세가 완성되면 "**수리모형부터 보여줘**"처럼 요청해서 Modeler가 세운 집합·변수·목적함수·
   제약을 확인하고, 우리가 말로 설명한 내용과 맞는지 살펴봅니다.
4. 이어서 "**gurobipy로 구현해줘**"라고 요청해 코드를 생성·실행받습니다.
5. 나온 **총 이익(목적함수 값)**과 각 설비 사용량이 아래 예상 결과와 일치하는지 비교합니다.
6. 만약 다르다면 무엇이, 왜 다른지(품질 제약의 부등호 방향, 설비 용량 제약 등) Modeler와의
   대화를 통해 찾아내고 수정을 요청해보세요.

### 검증 체크리스트

- [ ] `model.status`가 `OPTIMAL`(2)인가요?
- [ ] 총 이익(목적함수 값)이 약 **70,210.84**인가요?
- [ ] 원유 사용량이 상한(30,000배럴)에 도달했나요?
- [ ] 2차 가공 설비 사용량이 상한(5,000배럴)에 도달했나요?
- [ ] 단백질 농축 설비는 상한(6,000배럴)에 도달하지 **않고** 여유가 남나요?
